...
class User(db.Model):
    ...
    uid         = db.Column(db.String(36), default=uid()) # An immutable unique ID so that usernames can be changed
    ...
    def jsonify(self, with_groups=False, with_preferences=False):
        ...
        if self.last_login:
            response['last_login'] = time.mktime(self.last_login.timetuple())
...
class Event(db.Model):
    ...
    def jsonify(self):
        return {
            ...
            'time': time.mktime(self.created.timetuple()),
