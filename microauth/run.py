...
if __name__ == "__main__":
    ...
    if not options.key and not options.cert:
        ...
        print "$ openssl genrsa 4096 > key"
        print "$ openssl req -new -x509 -nodes -sha256 -days 365 -key key > cert"
        raise SystemExit
    ...
    if (pwd.getpwuid(os.getuid())[2] == 0 and (not options.run_as or options.run_as == "root")):
    ...
    if options.run_as:
        ...
        try:
            os.setuid(uid)
            print("Now running as %s." % options.run_as)
        except Exception, e: raise

    WSGIServer(sock, app, certfile=options.cert, keyfile=options.key).serve_forever()
